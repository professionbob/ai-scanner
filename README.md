...
每次 Action 的 job 上限為 14 分鐘，掃描器本身預留安裝與狀態保存時間，在 720 秒停止展開新標的。預設每輪從美股池取 20 檔、台股池取 30 檔，並分別保存 `us_scan_cursor` 與 `tw_scan_cursor`；下一次對應市場開盤時從上次位置接續，到尾端後環繞，逐輪覆蓋全市場。可用 `US_BATCH_SIZE`、`TW_BATCH_SIZE`、`MAX_RUN_SECONDS` 調整，但過大的批次會受到免費來源限流及 Action 時限影響。
