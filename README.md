...
新聞與動能是免費、best-effort 資料，可能延遲、限流、短暫缺漏或因未公告介面改版；Yahoo Finance 並非保證可用的即時行情服務。每個新聞與價格外部呼叫最多等待 15 秒，並會再縮短至本輪剩餘 deadline；deadline 已到時不再展開外部呼叫。**新聞抓取、基準或任何候選報價／解析只要失敗，整組浮動名單即原子性退回空名單，當輪仍照常執行既有安全結束與狀態保存，不會讓 Action 因浮動資料失敗。**
