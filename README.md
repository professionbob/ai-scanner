...
每次 Action 的 job 上限為 14 分鐘。`MAX_RUN_SECONDS` 預設 720 秒，但程式固定預留最後 90 秒供狀態保存及 runner 收尾，因此股票池下載、浮動更新與正式掃描共用的工作 deadline 預設是 630 秒。預設每輪從美股池取 20 檔、台股池取 30 檔，並分別保存 `us_scan_cursor` 與 `tw_scan_cursor`；游標只依實際完成且由市場批次選出的股票前進，優先股本身不額外推進游標。若途中超時，下一次對應市場開盤時會從第一個未完成的市場股票接續；到尾端後環繞，逐輪覆蓋全市場。可用 `US_BATCH_SIZE`、`TW_BATCH_SIZE`、`MAX_RUN_SECONDS` 調整，但過大的批次會受到免費來源限流及 Action 時限影響。
...
浮動名單每次最多美股 15 檔、台股 10 檔。程式以 Yahoo Finance 公開 search endpoint 對九個既有主題（AI 半導體、光通訊、記憶體、電力／核能、國防／無人機、機器人、資安、太空／衛星及 AI 生技）做有限次查詢，不逐檔大量抓新聞。只接受最近 72 小時內、具有 `relatedTickers` 明確代號關聯，且標題命中財報／營收、訂單／合約、併購、政策或分析師調整等催化類型的新聞；不以模糊公司名稱猜測股票。候選代號還必須存在於普通股市場名單，且通過與正式掃描一致的價格及 20 日平均成交量門檻（美股 US$5／500,000 股、台股 NT$10／100,000 股），因此 ETF、基金、權證、特別股、無效代號和低流動性標的不會入選。
...
新聞與動能是免費、best-effort 資料，可能延遲、限流、短暫缺漏或因未公告介面改版；Yahoo Finance 並非保證可用的即時行情服務。浮動更新另有最多 180 秒的子 deadline；最多先取 25 檔候選，再把候選與兩個市場基準合併為一次批量行情下載，不逐檔發出最多 50 個請求。每個新聞與行情外部呼叫最多等待 15 秒，並會再縮短至剩餘 deadline，而且呼叫前後都會檢查時間；deadline 已到時不再展開外部呼叫。**新聞抓取、基準或任何候選報價／解析只要失敗，整組浮動名單即原子性退回空名單，當輪仍照常執行既有安全結束與狀態保存，不會讓 Action 因浮動資料失敗。**
